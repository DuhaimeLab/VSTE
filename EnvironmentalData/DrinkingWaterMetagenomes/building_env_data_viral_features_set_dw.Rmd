...
# Virsorter2
vs2_path <- "DrinkingWaterToolOutput/merged_vs2_all_types_uknl.tsv"
# Kaiju
...
checkV <- checkV[!duplicated(checkV$contig),]
```
...
dvf_c <- dvf_c[!duplicated(dvf_c$contig),]
```
...
vs_c$contig <- paste(vs_c$assembly, vs_c$contig, sep = "--")
...
vs2_c <- vs2_c[!grep("seqname", vs2_c$contig),]
```
...
## Merging
```{r}
viruses <- left_join(x=vs2_c, y=checkV, by = c("contig"))
viruses <- left_join(x=viruses, y=dvf_c, by = c("contig", "assembly"))
viruses <- left_join(x=viruses, y=vb_c, by = c("contig"))
viruses <- left_join(x=viruses, y=vs_c, by = c("contig"))
viruses <- left_join(x=viruses, y=kj_c, by = c("contig", "assembly"))
```
...
viruses$RNA[grep("nan", viruses$RNA)] <- 0
viruses$lavidaviridae[is.na(viruses$lavidaviridae)] <- 0
viruses$lavidaviridae[grep("nan", viruses$lavidaviridae)] <- 0
viruses$NCLDV[is.na(viruses$NCLDV)] <- 0
viruses$NCLDV[grep("nan", viruses$NCLDV)] <- 0
viruses$ssDNA[is.na(viruses$ssDNA)] <- 0
viruses$ssDNA[grep("nan", viruses$ssDNA)] <- 0
viruses$dsDNAphage[is.na(viruses$dsDNAphage)] <- 0
viruses$dsDNAphage[grep("nan", viruses$dsDNAphage)] <- 0
viruses$viral[is.na(viruses$max_score)] <- 0
...
#remove contigs flagged as "less than 2 genes by vs2
viruses <- viruses[!viruses$vs2type=="lt2gene",]
```
...
write_tsv(viruses, paste(dataset_name, viruses_filename, sep='_'))
```
