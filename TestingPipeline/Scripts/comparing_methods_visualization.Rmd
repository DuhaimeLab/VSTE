...
getting_viral_set_1 <- function(input_seqs,
                                ...
                                tv_1=T, tv_2=T, tv_3=T, tv_4=T, tv_5=T,
...
tuning_viral <- function(current_score, ins, 
  ...
  if (include_tv_5) {
    current_score[(ins$checkv_completeness>=75 | ins$vibrant_quality=="complete circular") & (ins$viral>=50 | ins$percent_viral>=50)] <- current_score[(ins$checkv_completeness>=75 | ins$vibrant_quality=="complete circular") & (ins$viral>=50 | ins$percent_viral>=50)] + 0.5 
  }
...
ggplot(prop_high_viruses_long[prop_high_viruses_long$vs2==1,], aes(x=as.factor(numtools), y=proportion, 
  ...
  ylab("Proportion of Contigs in Common") +
  scale_fill_manual(name="",
                     values = alpha(viridis(6), 0.3)) +
  scale_color_manual(name="",
                     values = alpha(viridis(6), 0.7))
...
comparing VS and VS2 viruses
```{r}
vs2_vs <- data.frame(vs2=viral_scores_two[,1],
                     vs=viral_scores_two[,2],
                     comb_score=viral_scores_two[,1]+viral_scores_two[,2],
                     seqtype=viruses$seqtype,
                     max_score_group=viruses$max_score_group)

vs2_vs$comb[vs2_vs$comb_score<1] <- "neither"
vs2_vs$comb[vs2_vs$vs2==0.5 & vs2_vs$vs==0.5] <- "both_low"
vs2_vs$comb[vs2_vs$vs2>=1 & vs2_vs$vs==0.5] <- "vs2_high_vs_low"
vs2_vs$comb[vs2_vs$vs2>=0.5 & vs2_vs$vs>=1] <- "vs2_low_vs_high"
vs2_vs$comb[vs2_vs$vs2>=1 & vs2_vs$vs==0] <- "vs2_only"
vs2_vs$comb[vs2_vs$vs2==0 & vs2_vs$vs>=1] <- "vs_only"
vs2_vs$comb[vs2_vs$vs2>=1 & vs2_vs$vs>=1] <- "both_high"

#vs2_vb$true_virus <- "not"
#vs2_vb$true_virus[vs2_vb$seqtype=="virus"] <- "virus"

vs2_vs_seqtype <- vs2_vs %>% 
...
```{r}
ggplot(vs2_vs_seqtype, aes(y=n, x=comb,
...
```{r}
ggplot(vs2_vb_seqtype, aes(y=n, x=seqtype,
                   fill=as.factor(comb_score))) +
  geom_bar(stat="identity", color="black") +
  theme_light() +
  coord_flip() +
  ...
  xlab("") +
  ylab("Number of Sequences") + 
  facet_grid(comb~max_score_group, scales = "free")
```

looking at the max_score_group
```{r}
ggplot(vs2_vb_seqtype[vs2_vb_seqtype$seqtype=="virus",], aes(y=n, x=comb,
                   fill=as.factor(comb_score))) +
  geom_bar(stat="identity", color="black") +
  theme_light() +
  coord_flip() +
...
vs2_dvf_seqtype <- vs2_dvf %>% 
  group_by(comb, comb_score, seqtype, max_score_group) %>%
  summarise(n = n())
```

looking at the max_score_group
```{r}
ggplot(vs2_dvf_seqtype, aes(y=n, x=seqtype,
                   fill=as.factor(comb_score))) +
  geom_bar(stat="identity", color="black") +
  theme_light() +
  coord_flip() +
...
comparing VS2 and tv viruses
```{r}
vs2_tv <- data.frame(vs2=viral_scores_two[,1],
                     tv=viral_scores_two[,4],
                     comb_score=viral_scores_two[,1]+viral_scores_two[,4],
                     seqtype=viruses$seqtype,
                     max_score_group=viruses$max_score_group)

vs2_tv$comb[vs2_tv$comb_score<1] <- "neither"
vs2_tv$comb[vs2_tv$vs2==0.5 & vs2_tv$tv==0.5] <- "both_low"
vs2_tv$comb[vs2_tv$vs2>=1 & vs2_tv$tv==0.5] <- "vs2_high_tv_low"
vs2_tv$comb[vs2_tv$vs2>=0.5 & vs2_tv$tv>=1] <- "vs2_low_tv_high"
vs2_tv$comb[vs2_tv$vs2>=1 & vs2_tv$tv==0] <- "vs2_only"
vs2_tv$comb[vs2_tv$vs2==0 & vs2_tv$tv>=1] <- "tv_only"
vs2_tv$comb[vs2_tv$vs2>=1 & vs2_tv$tv>=1] <- "both_high"

vs2_tv_seqtype <- vs2_tv %>% 
  group_by(comb, comb_score, seqtype, max_score_group) %>%
  summarise(n = n())
```
...
looking at the max_score_group
```{r}
ggplot(vs2_tv_seqtype, aes(y=n, x=seqtype,
                   fill=as.factor(comb_score))) +
  geom_bar(stat="identity", color="black") +
  theme_light() +
  coord_flip() +
...
comparing VS2 and tnv viruses
```{r}
vs2_tnv <- data.frame(vs2=viral_scores_two[,1],
                     tnv=viral_scores_two[,6],
                     comb_score=viral_scores_two[,1]+viral_scores_two[,6],
                     seqtype=viruses$seqtype,
                     max_score_group=viruses$max_score_group)

vs2_tnv$comb[vs2_tnv$comb_score<1] <- "both_not"
vs2_tnv$comb[vs2_tnv$vs2>=1 & vs2_tnv$tnv<=0] <- "vs2_only"

vs2_tnv_seqtype <- vs2_tnv %>% 
  group_by(comb, comb_score, seqtype, max_score_group) %>%
  summarise(n = n())
```
...
looking at the max_score_group
```{r}
ggplot(vs2_tnv_seqtype, aes(y=n, x=seqtype,
                   fill=as.factor(comb_score))) +
  geom_bar(stat="identity", color="black") +
  theme_light() +
  coord_flip() +
...
comparing rule set individually
```{r}
ind <- data.frame(vs2_scores=viral_scores_two[,1],
                  vs_scores=viral_scores_two[,2],
                  vb_scores=viral_scores_two[,3],
                  tv_scores=viral_scores_two[,4],
                  dvf_scores=viral_scores_two[,5],
                  tnv_scores=viral_scores_two[,6],
                  seqtype=viruses$seqtype
                     )

ind_scores <- ind %>% 
  select(contains("scores"), seqtype) %>%
  pivot_longer(cols=contains("_scores"), 
               names_to="rule_set",
               values_to="viral_score") %>% 
  mutate(viral_score=as.factor(round(viral_score))) %>%
  group_by(rule_set, viral_score, seqtype) %>%
  summarise(n = n())
```


```{r}
ggplot(ind_scores, aes(y=n, x=rule_set,
                   fill=as.factor(viral_score))) +
  geom_bar(stat="identity", color="black") +
  theme_light() +
  coord_flip() +
  theme(
...
recall
```{r}
length(grep("true positive", viruses_refseq_vir$))/nrow(viruses_refseq_vir)
```
