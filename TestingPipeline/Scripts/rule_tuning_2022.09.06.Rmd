...
ggplot(accuracy_scores[accuracy_scores$testing_set_index==1,], aes(x=precision, y=recall)) +
  ...
  geom_point(alpha=0.5, aes(color=numtools, fill=numtools)) +
  theme_light() +
  theme(
    panel.grid.major.y = element_blank(),
    panel.border = element_blank(),
...
stats for different types of sequences
```{r}
TP <- table(viruses$confusion_matrix_high_MCC)[4]
FN <- table(viruses$confusion_matrix_high_MCC)[1]
...
```{r}
head(viruses$contig[viruses$seqtype=="virus"])
tail(viruses$contig[viruses$seqtype=="virus"])
```
