...
only 5 testing sets   
```{r}
viruses <- viruses %>% filter(Index<=5)
```

balancing testing set
1000 viruses per testing set
600 bacteria
100 arachaea
100 fungi
100 plasmid
100 protist
```{r}
viruses <- rbind(viruses %>% filter(seqtype=="archaea") %>%
  group_by(Index) %>%
  slice_sample(n=100),
  viruses %>% filter(seqtype=="bacteria") %>%
  group_by(Index) %>%
  slice_sample(n=600),
  viruses %>% filter(seqtype=="fungi") %>%
  group_by(Index) %>%
  slice_sample(n=100),
  viruses %>% filter(seqtype=="plasmid") %>%
  group_by(Index) %>%
  slice_sample(n=100),
  viruses %>% filter(seqtype=="protist") %>%
  group_by(Index) %>%
  slice_sample(n=100),
  viruses %>% filter(seqtype=="virus") %>%
  group_by(Index) %>%
  slice_sample(n=1000)
)
...
```{r}
t.test(accuracy_scores$MCC[accuracy_scores$toolcombo=="1 0 1 0 0 1"], 
...
t.test(accuracy_scores$MCC[accuracy_scores$toolcombo=="1 0 1 0 0 1"], 
       accuracy_scores$MCC[accuracy_scores$toolcombo=="1 1 1 0 1 1"])

t.test(accuracy_scores$MCC[accuracy_scores$toolcombo=="1 0 1 0 0 1"], 
       accuracy_scores$MCC[accuracy_scores$toolcombo=="1 0 1 1 1 1"])

t.test(accuracy_scores$MCC[accuracy_scores$toolcombo=="1 0 1 0 0 1"], 
       accuracy_scores$MCC[accuracy_scores$toolcombo=="1 0 0 0 0 1"])

t.test(accuracy_scores$MCC[accuracy_scores$toolcombo=="1 0 1 0 0 1"], 
       accuracy_scores$MCC[accuracy_scores$toolcombo=="1 1 1 1 0 1"])

t.test(accuracy_scores$MCC[accuracy_scores$toolcombo=="1 0 1 0 0 1"], 
       accuracy_scores$MCC[accuracy_scores$toolcombo=="1 1 1 1 1 1"])
```
...
pheatmap::pheatmap(as.matrix(viral_scores_1),
                  ...
                  cluster_cols = F,
                  cellheight=3,
                  filename = "../IntermediaryFiles/heatmap_MCC_labels_50-50.png"
                  )
```
...
```{r}
viruses$keep_score_all_tv_only_4 <- getting_viral_set_1(viruses, include_deepvirfinder = F,
                                              include_vibrant = F,
                                              include_virsorter2 = F,
                                              include_tuning_viral = T,
...
viruses$keep_score_all_tv_only_1 <- getting_viral_set_1(viruses, include_deepvirfinder = F,
                                              include_vibrant = F,
                                              include_virsorter2 = F,
                                              include_tuning_viral = T,
                                              tv_1=T, tv_2=F, tv_3=F, tv_4=F,
...
viruses$keep_score_all_tv_only_2 <- getting_viral_set_1(viruses, include_deepvirfinder = F,
                                              include_vibrant = F,
                                              include_virsorter2 = F,
                                              include_tuning_viral = T,
...
viruses$keep_score_all_tv_only_3 <- getting_viral_set_1(viruses, include_deepvirfinder = F,
                                              include_vibrant = F,
                                              include_virsorter2 = F,
                                              include_tuning_viral = T,
...
viruses_long_scores <- viruses %>% 
  select(contains("keep_score_all_tv_only")) 

viruses_long_scores <- apply(viruses_long_scores, c(1,2), function(x) {if (x>0){x<-1} else{x<-0}})

viruses_long_scores <- as.data.frame(cbind(viruses$true_virus, rowSums(viruses_long_scores)))

table(viruses_long_scores$V1, viruses_long_scores$V2>1)
```
...
ann_colors = list(
  ...
  vs = c("black", pal(6)[5]),
  vs2 = c("black", pal(6)[6]),
  score = c("archaea"=pal2(20)[13], "bacteria"=pal2(20)[9], "fungi"=pal2(20)[4],
...
pheatmap::pheatmap(t(viral_scores_1),
                  ...
                  cluster_cols = F,
                  cellheight=3,
                  filename = "../IntermediaryFiles/pheatmap_virus_set_1_viral_scores_balanced.png"
                  )
```
