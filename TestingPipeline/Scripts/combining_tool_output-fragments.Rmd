---
title: "Combining Tool Output - Fragments"
...
dataset_name <- "testing_set-fragments"
# checkV
checkV_path <- "../ToolOutput/Fragments_Results/merged_checkv_assemblies.tsv"
# VIBRANT
vibrant_path <- "../ToolOutput/Fragments_Results/merged_vibrant_assemblies.tsv"
# DeepVirFinder
dvf_path <- "../ToolOutput/Fragments_Results/merged_dvf_assemblies.tsv"
# Virsorter
vs_path <- "../ToolOutput/Fragments_Results/merged_virsorter_assemblies.tsv"
# Virsorter2
vs2_path <- "../ToolOutput/Fragments_Results/merged_vs2_assemblies.tsv"
# Kaiju
kj_path <- "../ToolOutput/Fragments_Results/merged_kaiju_assemblies.tsv"
# KB cutoff
KB_CUTOFF <- 3000
```
...
vs_c <- vs_c[!grep("#", vs_c$contig),]
vs_c <- separate(vs_c, col = `contig`, into = c("Index", "type", "contig"), sep="--")
vs_c$seqtype <- sub("VIRSorter_", "", vs_c$type)
vs_c$contig <- sub("\\.", "_", vs_c$contig)
vs_c <- vs_c %>% drop_na(contig)
vs_c <- vs_c[!duplicated(paste(vs_c$contig, vs_c$Index)),]
vs_c$Index <- as.numeric(vs_c$Index)
...
colnames(kj_c)[2] <- "Contig"
colnames(kj_c)[ncol(kj_c)] <- "Index"
...
```{r}
write_tsv(viruses, "../IntermediaryFiles/viral_tools_combined-fragments.tsv")
```