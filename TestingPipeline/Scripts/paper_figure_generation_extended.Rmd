...
getting_viral_set_2 <- function(input_seqs,
  ...
  if (include_virsorter2) {
   # keep_score[input_seqs$max_score>=0.50] <- keep_score[input_seqs$max_score>=0.50] + 0.5
    keep_score[input_seqs$max_score>=0.95] <- keep_score[input_seqs$max_score>=0.95] + 1  
  }
  ...
  if (include_deepvirfinder) {
    ...
    keep_score[(input_seqs$score>=0.9 & input_seqs$pvalue<=0.05) & input_seqs$checkv_length<20000] <- keep_score[(input_seqs$score>=0.9 & input_seqs$pvalue<=0.05) & input_seqs$checkv_length<20000] + 1
...
accuracy_scores <- data.frame(testing_set_index=rep(0, nrow(combos_list)*5),
                      ...
                      prop_viral=rep(0, nrow(combos_list)*5))
...
#comparing based on 0.5 rules or not

```{r}
#accuracy_scores_melt_no_0.5 <- accuracy_scores_melt
```


```{r}
percent_diff <- data.frame(toolcombo=accuracy_scores_melt$toolcombo,
                           percent_diff=(accuracy_scores_melt$performance_metric_score -
                   accuracy_scores_melt_no_0.5$performance_metric_score)/
  accuracy_scores_melt$performance_metric_score,
                          performance_metric=accuracy_scores_melt$performance_metric)
)


fig <- ggplot(percent_diff, 
              aes(x=performance_metric, y=percent_diff)) +
  geom_boxplot() +
  #geom_point() +
  theme_light() +
  theme(
...
```{r}
t.test(accuracy_scores_melt$performance_metric_score[accuracy_scores_melt$performance_metric=="MCC"],
       accuracy_scores_melt_no_0.5$performance_metric_score[accuracy_scores_melt_no_0.5$performance_metric=="MCC"])
```

```{r}
t.test(accuracy_scores_melt$performance_metric_score[accuracy_scores_melt$performance_metric=="precision"],
       accuracy_scores_melt_no_0.5$performance_metric_score[accuracy_scores_melt_no_0.5$performance_metric=="precision"])
```

```{r}
t.test(accuracy_scores_melt$performance_metric_score[accuracy_scores_melt$performance_metric=="recall"],
       accuracy_scores_melt_no_0.5$performance_metric_score[accuracy_scores_melt_no_0.5$performance_metric=="recall"])
```

```{r}
t.test(accuracy_scores_melt$performance_metric_score[accuracy_scores_melt$performance_metric=="prop_viral"],
       accuracy_scores_melt_no_0.5$performance_metric_score[accuracy_scores_melt_no_0.5$performance_metric=="prop_viral"])
```
