...
colnames(MCC_sd_quant) <- c("testing_set_index", "2", "3", "4",
                            "5", "6", "7", "8")

MCC_sd_quant <- MCC_sd_quant %>% pivot_longer(cols=!contains("testing_set_index"))
...
ggplot(MCC_sd_quant, aes(x=name, y=value)) +
  ...
  xlab("Number of Testing Sets Included") +
  ylab("Standard Deviation") 
```
```{r}
t.test(MCC_sd_quant$value[MCC_sd_quant$name==2],MCC_sd_quant$value[MCC_sd_quant$name==3])
t.test(MCC_sd_quant$value[MCC_sd_quant$name==3],MCC_sd_quant$value[MCC_sd_quant$name==4])
t.test(MCC_sd_quant$value[MCC_sd_quant$name==4],MCC_sd_quant$value[MCC_sd_quant$name==5])
t.test(MCC_sd_quant$value[MCC_sd_quant$name==5],MCC_sd_quant$value[MCC_sd_quant$name==6])
t.test(MCC_sd_quant$value[MCC_sd_quant$name==6],MCC_sd_quant$value[MCC_sd_quant$name==7])
t.test(MCC_sd_quant$value[MCC_sd_quant$name==7],MCC_sd_quant$value[MCC_sd_quant$name==8])
```
...
mean_precision <- accuracy_scores %>% 
  ...
  group_by(toolcombo) %>%
  dplyr::summarise(mean = mean(precision)) %>%
  arrange(mean)
```


```{r}
accuracy_scores$toolcombo <- factor(accuracy_scores$toolcombo, levels = mean_precision$toolcombo)
...
fig <- ggplot(accuracy_scores, aes(x=toolcombo, y=precision, 
                                  color=ruletype, fill=ruletype)) +
...
ggsave(
  "../IntermediaryFiles/all_precision_scatterplot.png",
  plot = fig,
  scale = 1,
  width = 8,
  height = 4,
...
viral_scores_1 <- mean_precision
rn <- viral_scores_1$toolcombo

viral_scores_1 <- viral_scores_1 %>% select(c("mean"))
viral_scores_1 <- as.matrix(viral_scores_1)
#viral_scores_1 <- viral_scores_1 %>% select(c("mean"))
rownames(viral_scores_1) <- rn

pheatmap::pheatmap(viral_scores_1,
                  ...
                  cluster_cols = F,
                  cellheight=3,
                  filename = "../IntermediaryFiles/heatmap_precision_labels_S2A.png"
                  )
```
...
mean_recall <- accuracy_scores %>% 
  ...
  group_by(toolcombo) %>%
  dplyr::summarise(mean = mean(recall)) %>%
  arrange(mean)
```
...
accuracy_scores$toolcombo <- factor(accuracy_scores$toolcombo, levels = mean_recall$toolcombo)
...
fig <- ggplot(accuracy_scores, aes(x=toolcombo, y=recall, 
                                  color=ruletype, fill=ruletype)) +
  geom_boxplot(alpha=0.5) +
  theme_light() +
  theme(
...
ggsave(
  "../IntermediaryFiles/all_recall_scatterplot.png",
  plot = fig,
  scale = 1,
  width = 8,
  height = 4,
...
making heatmap for the x-axis label - Figure S2B
```{r}
ann_col = data.frame(
    tnv = combos_list$tune_not_viral,
    tv = combos_list$tune_viral,
...
#viral_scores_1 <- accuracy_scores %>% filter(testing_set_index==5)
viral_scores_1 <- mean_recall
rn <- viral_scores_1$toolcombo

#viral_scores_1 <- viral_scores_1 %>% select(c("recall"))
viral_scores_1 <- viral_scores_1 %>% select(c("mean"))
rownames(viral_scores_1) <- rn
...
pheatmap::pheatmap(as.matrix(viral_scores_1),
                  ...
                  cluster_cols = F,
                  cellheight=3,
                  filename = "../IntermediaryFiles/heatmap_recall_labels_S2B.png"
                  )
```
...
fig <- ggplot(accuracy_scores_melt[accuracy_scores_melt$toolcombo!="0 0 1 0 0 0" & accuracy_scores_melt$performance_metric!="prop_viral",], aes(x=numrules, y=performance_metric_score)) +
  ...
  geom_point(alpha=0.2, fill="black", color="black") +
  theme_light() +
  theme(
    panel.grid.major.y = element_blank(),
    panel.border = element_blank(),
...
ggsave(
  "../IntermediaryFiles/MCC_precision_recall_boxplots_grey.png",
  plot = fig,
  scale = 1,
  width = 8,
  height = 4,
...
pheatmap::pheatmap(prop_high_viruses,
                  ...
                  annotation_colors = ann_colors,
                  border_color = "black",
                  color = c(viridis(4)[1], rep(viridis(4)[2], 4), rep(viridis(4)[3], 4), viridis(4)[4]),
                  cutree_rows = 3,
                  cutree_cols = 3,
                  show_rownames = F,
                  show_colnames = F,
                  filename = "../IntermediaryFiles/pheatmap_prop_viruses_in_common_4_breaks.png",
                  annotation_legend = T)
...
pheatmap::pheatmap(prop_high_viruses,
                  ...
                  show_rownames = F,
                  show_colnames = F,
                  filename = "../IntermediaryFiles/pheatmap_prop_viruses_in_common_5_breaks.png",
                  annotation_legend = T)
```
...
```{r}
viruses_from_tv <- getting_viral_set_1(viruses, include_deepvirfinder = F,
                                              include_vibrant = F,
                                              include_virsorter2 = F,
                                              include_tuning_viral = T,
                                              include_tuning_not_viral = F,
...
table(viruses_from_tv, viruses$seqtype)
```

```{r}
viruses_from_ntv <- getting_viral_set_1(viruses, include_deepvirfinder = F,
                                              include_vibrant = F,
                                              include_virsorter2 = F,
                                              include_tuning_viral = F,
                                              include_tuning_not_viral = T,
...
table(viruses_from_ntv, viruses$seqtype)
```
